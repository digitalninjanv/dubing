...
impl MainWindow {
    pub fn build(
        ...
        dropzone_view.connect_translate_clicked(move || {
            let needs_gemini = settings_exec.providers.transcriber.eq_ignore_ascii_case("gemini")
                || settings_exec.providers.translator.eq_ignore_ascii_case("gemini")
            ...
            let api_key = if needs_gemini {
                match secret_store_exec.get_api_key() {
                    ...
                    _ => {
                        let toast =
                            libadwaita::Toast::new("Configure the Gemini API key in Settings or GEMINI_API_KEY");
